...
const DELETE_ADMIN_FAQ = gql`
  mutation deleteAdminFaq($id: String) {
    deleteAdminFaq(id: $id) {
      ok
    }
  }
`;
...
const AdminFaqDetail = () => {
  ...
  const { data } = useQuery(SEE_ADMIN_FAQ_QUERY, {
    variables: {
  ...
  const [deleteAdminFaqMutation] = useMutation(DELETE_ADMIN_FAQ, {
    onCompleted,
  });

  const onDelete = () => {
    if (window.confirm("정말 삭제하시겠습니까?")) {
      deleteAdminFaqMutation({
        variables: {
          id: location.state.id,
        },
      });
  ...
  let getDate = new Date(parseInt(data?.seeAdminFaq.createdAt));
  ...
  return (
    <BannerDetailContainer>
      ...
      <BannerContent>
        <BannerBtnWrap>
          <BannerInputBtn onClick={() => navigate("/adminfaq")}>
            목록
          </BannerInputBtn>
          <BannerInputBtn
            onClick={() =>
              navigate("/adminfaq/editAdminFaq", {
        ...
        <BannerTitle>
          ...
          <ColumnDisc>{data?.seeAdminFaq.title}</ColumnDisc>
        </BannerTitle>
        <BannerDate>
        ...
        <BannerDiscription>
          ...
          <ColumnDisc
            dangerouslySetInnerHTML={{
              __html: data?.seeAdminFaq.discription,
            }}
          ></ColumnDisc>
        </BannerDiscription>
      </BannerContent>
