...
const EDIT_ADMIN_NOTICE_MUTATION = gql`
  mutation editAdminNotice($id: String, $title: String, $discription: String) {
    editAdminNotice(id: $id, title: $title, discription: $discription) {
      ok
      error
    }
  }
...
function EditAdminNotice() {
  const navigate = useNavigate();
  const location = useLocation();

  const { data, loading } = useQuery(SEE_ADMIN_NOTICE_QUERY, {
    variables: {
  ...
  const [contents, setContents] = useState(data?.seeAdminNotice.discription);
  const [title, setTitle] = useState(data?.seeAdminNotice.title);

  const onCompleted = (data: any) => {
    const {
      editAdminNotice: { ok, error },
    } = data;
  ...
  useEffect(() => {
    setValue("id", data?.seeAdminNotice.id);
    setValue("title", data?.seeAdminNotice.title);
    setValue("discription", data?.seeAdminNotice.discription);
  }, []);

  return (
    <Container>
      <PageTitle title="공지사항 수정" />
      <HeaderTitle title="공지사항 수정" />
      <BoardHeader />
      <InputContainer>
...
export default EditAdminNotice;
