...
const EDIT_CONTEST_NOTICE_MUTATION = gql`
  mutation editContestNotice(
    $id: String
    $noticeTitle: String
...
const SEE_CONTEST_NOTICE_DETAIL = gql`
  query seeContestNotice($id: String) {
    seeContestNotice(id: $id) {
      id
      noticeTitle
      noticeDiscription
    }
...
export default function EditContestNotice() {
  const inputRef = useRef<HTMLInputElement | null>(null);
  const [contents, setContents] = useState("");
  const navigate = useNavigate();
  const location = useLocation();

  const { data: noticeData } = useQuery(SEE_CONTEST_NOTICE_DETAIL, {
    variables: {
  ...
  const [editContestNoticeMutation] = useMutation<any>(
    EDIT_CONTEST_NOTICE_MUTATION,
    {
      onCompleted,
    }
  );
  ...
  useEffect(() => {
    setTitle(noticeData?.seeContestNotice.noticeTitle);
    setDisc(noticeData?.seeContestNotice.noticeDiscription);
  }, [noticeData]);
        ...
        <form onSubmit={handleSubmit(onSubmitValid)}>
          <TitleInput
            ...
            onChange={(e) => {
              ...
              setValue("noticeTitle", e.target.value);
            }}
          />
          <TextEditor>
            <ReactQuill
