...
const SEE_EDIT_FACILITY_QUERY = gql`
...
const EDIT_FACILITY_MUTATION = gql`
...
function EditFacility() {
  ...
  const [editFacilityMutation] = useMutation<createFacilityVariables>(
    EDIT_FACILITY_MUTATION,
    {
      onCompleted,
    }
  );

  const { data: facilityData, loading } = useQuery(SEE_EDIT_FACILITY_QUERY, {
    variables: {
  ...
  // 시설 종목 체크박스 세팅
  let eventList = useSportsEvent(facilityData?.seeFacility?.facilitySports);
  ...
  // 시설 태그 체크박스 세팅
  let tagList = useTag("facility", facilityData?.seeFacility?.facilityTag);
  ...
  useEffect(() => {
    setFacilityName(facilityData?.seeFacility?.name);
    setDiscription(facilityData?.seeFacility?.discription);
    setOperTime(facilityData?.seeFacility?.operTime);
    onSportsEventPress(undefined);
    onTagList(undefined);
    setFacilityInfo(facilityData?.seeFacility?.facilityInfo);
  }, [loading]);
        ...
        <form onSubmit={handleSubmit(onSubmitValid)}>
          ...
          <TitleFacilityContainer>
            ...
            <TitleInput
              ...
              value={operTime}
              onChange={(e) => setOperTime(e.target.value)}
            />
          </TitleFacilityContainer>
          <TitleFacilityContainer>
          ...
          <TitleFacilityContainer>
            ...
            <TitleInput
              ...
              onChange={() =>
                setSelectDay(moment(selectDay).format("YYYY.MM.DD"))
              }
