...
const SEE_CONTEST_REPORT_LIST = gql`
  query seeContestReports($contestId: String) {
    seeContestReports(contestId: $contestId) {
      id
      reportTitle
      reportDiscription
      createdAt
...
export default function ContestNoticeTable() {
  ...
  const { data: reportData } = useQuery(SEE_CONTEST_REPORT_LIST, {
    variables: {
  ...
  return (
    <Container>
      <BannerBtnWrap>
        ...
        <BannerInputBtn
          onClick={() =>
            navigate("/contest/createContestReport", {
      ...
      <TableContainer component={Paper} style={{ padding: 16 }}>
        <Table size="small">
          ...
          <TableBody>
            {reportData?.seeContestReports
              .slice(page * rowsPerPage, (page + 1) * rowsPerPage)
              .map(
                (
                  { id, reportTitle, reportDiscription, createdAt }: any,
                  i: number
                ) => {
                  ...
                  return (
                    <TableRow key={id}>
                      ...
                      <TableCell
                        ...
                        onClick={() =>
                          navigate("/contest/contestReportDetail", {
                            state: { id, reportTitle, reportDiscription },
                          })
                        }
                      >
                        {reportTitle}
          ...
          <TableFooter>
            <TableRow>
              <TablePagination
                count={reportData?.seeContestReports.length}
                page={page}
